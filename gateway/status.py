...
def read_scoped_lock(scope: str, identity: str) -> Optional[dict[str, Any]]:
    """Read the current lock payload for ``scope``/``identity`` if present."""
    return _read_json_file(_get_scope_lock_path(scope, identity))


def force_release_scoped_lock(
    scope: str,
    identity: str,
    *,
    ...
    """Force-release a scoped lock, optionally terminating the owning process.

    Returns:
        (True, record, reason) on success, where ``record`` is the previous lock
        payload (if any) and reason is one of: ``missing``, ``released``.
        (False, record, reason) when a live owner could not be terminated or the
    ...
    # PID reuse or stale metadata: treat as stale and release.
    if owner_alive and not owner_matches_record:
        owner_alive = False

    if owner_alive and existing_pid == os.getpid() and existing.get("start_time") == _get_process_start_time(os.getpid()):
        owner_alive = False

    if owner_alive:
        if not terminate_owner:
            return False, existing, "owner_alive"
        try:
            terminate_pid(existing_pid, force=force_kill)
        except (ProcessLookupError, PermissionError, OSError) as exc:
            return False, existing, f"terminate_failed:{exc}"

        # Verify the process is gone before removing the lock.
        try:
            os.kill(existing_pid, 0)
            return False, existing, "owner_still_alive"
        except (ProcessLookupError, PermissionError):
            pass
    ...
    try:
        ...
        return False, existing, f"unlink_failed:{exc}"
    return True, existing, "released"
