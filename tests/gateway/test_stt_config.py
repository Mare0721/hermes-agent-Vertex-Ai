...
@pytest.mark.asyncio
async def test_enrich_message_with_video_visual_context_prepends_summary():
    from gateway.run import GatewayRunner

    runner = GatewayRunner.__new__(GatewayRunner)

    async def _fake_summary(_path):
        return {
            "success": True,
            "summary": "- Frame 1: shared whiteboard and architecture notes",
        }
    ...
    with patch(
        ...
        result = await runner._enrich_message_with_video_visual_context(
            "caption",
            ["/tmp/demo.mp4"],
        )

    assert "Sampled visual context from key frames" in result
    assert "shared whiteboard" in result
    assert "caption" in result


@pytest.mark.asyncio
async def test_prepare_inbound_message_text_includes_video_visual_context():
    from gateway.run import GatewayRunner
    ...
    async def _fake_summary(_path):
        return {
            "success": True,
            "summary": "- Frame 1: terminal window with test logs",
        }
    ...
    assert result is not None
    assert "Sampled visual context from key frames" in result
    assert "terminal window with test logs" in result
    assert "video voice transcript" in result
