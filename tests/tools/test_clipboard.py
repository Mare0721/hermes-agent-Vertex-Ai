...
# ═════════════════════════════════════════════════════════════════════════
# Level 2: _preprocess_videos_with_transcription — video -> vision+audio
# ═════════════════════════════════════════════════════════════════════════

class TestPreprocessVideosWithTranscription:
    """Test video pre-processing for visual frame sampling + audio transcription."""
    ...
    def _make_video(self, tmp_path, name="clip.mp4"):
        video = tmp_path / name
        video.write_bytes(b"not-a-real-video")
        return video

    def test_video_preprocess_includes_visual_summary_and_transcript(self, cli, tmp_path):
        video = self._make_video(tmp_path)

        async def _fake_video_summary(_path):
            return {
                "success": True,
                "summary": "- Frame 1: dashboard with latency chart",
            }

        with patch(
            "tools.video_enrichment.summarize_video_visual_context",
            side_effect=_fake_video_summary,
        ), patch(
            "tools.transcription_tools.transcribe_audio",
            return_value={"success": True, "transcript": "hello from the demo"},
        ):
            result = cli._preprocess_videos_with_transcription(
                "Please summarize",
                [video],
                announce=False,
            )

        assert "Sampled visual context" in result
        assert "dashboard with latency chart" in result
        assert "hello from the demo" in result
        assert "Please summarize" in result

    def test_video_preprocess_keeps_path_when_visual_or_stt_unavailable(self, cli, tmp_path):
        video = self._make_video(tmp_path)
        ...
        assert "frame-based visual analysis was unavailable" in result
        assert "automatic transcription failed" in result
        assert "Need details" in result
